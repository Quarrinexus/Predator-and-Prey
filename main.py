...
class Simulation(mp.Process):
    def __init__(self, new_simulation=True, width=1800, height=1200):
        super().__init__()
            ...
            if pred.closest_prey is not None:
                ...
                if dist < 20:
                    ...
                    if len(matches) > 0:
                        ...
                        weakest_fittest_fitness = min(self.fittest_preys, key=lambda x: x.fitness, default=None).fitness
    ...
    def start_new_simulation(self, total_starting_prey=50, total_starting_predators=50):
        ...
        self.fittest_predators = np.array(list(self.preys)[0:10], dtype=object)
    ...
    def run(self):
        ...
        while running:
            ...
            # First predator in list is temporarily the brain displayed by renderer
            if queue.empty():
                if len(self.predators) > 0:
                    queue.put(self.predators.sprites()[0].brain)
                else:
                    queue.put(None)         
            ...
            if counter == 30:
                ...
                plotting.gather_population_data(len(self.preys), len(self.predators))
            elif counter == 15:
                predators, fittest_predators = self.remove_starved_entities(self.predators, self.fittest_predators)
                if len(predators) == 0:
                    self.predators = self.prevent_extinction(predators, fittest_predators)
            ...
            pygame.display.flip()
            #t1 = tm.time()
            #self.logger.info(f"Frame time: {t1 - t0} seconds")
...
def main():
    global queue
    queue = mp.Queue()
    simulation = Simulation(new_simulation=True)
    side_gui = Brain_GUI(queue)
    simulation.start()
    side_gui.start()
    simulation.join()
    side_gui.join()
...
if __name__ == "__main__":
    #import cProfile
    #cProfile.run('main()', 'profile.out') # use snakeviz to view the profile data
    main()
